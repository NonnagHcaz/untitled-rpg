...
class Header:
    ...
    def __init__(self, console, width, config={}):
        ...
        self.layout_name = (
            self.layout[0]
            if len(self.layout) > 0 and self.layout[0] in Header.LAYOUTS
            else "TEXT_LEFT"
        )
        ...
        self.surf_dim = pygame.Rect(
            0, 0, self.width, self.line_spacing + self.padding[0] + self.padding[1]
        )
        ...
        if self.bck_image:
            ...
            if self.bck_image_resize:
                self.bck_image = pygame.transform.scale(
                    self.bck_image, (self.surf_dim.width, self.surf_dim.height)
                )
        ...
        self.txt_surf = pygame.Surface(
            (self.txt_surf_dim.width, self.txt_surf_dim.height), pygame.SRCALPHA
        )

        #####
        # Create surface for text and store its dimensions
        #####
        (self.fnt_txt_surf, self.fnt_txt_surf_dim) = self.font_object.render(
            self.text, self.font_color, None
        )
        ...
        if self.font_bck_color:
            ...
            self.fnt_bck_surf = pygame.Surface(
                (self.fnt_txt_surf_dim.width, self.line_spacing)
            )
        ...
        if self.layout_name in ["SCROLL_LEFT_CONTINUOUS", "SCROLL_RIGHT_CONTINUOUS"]:
            self.scroll_repeats = (
                self.txt_surf_dim.width // self.fnt_txt_surf_dim.width
            ) + 2
    ...
    def update(self):
        ...
        if self.text_params:
            ...
            text = self.text.format(
                *[
                    getattr(self.console.app, method_name)()
                    for method_name in self.text_params
                ]
            )

            # generate the new text in self.text_surface object
            (self.fnt_txt_surf, self.fnt_txt_surf_dim) = self.font_object.render(
                text, self.font_color, None
            )

            # How many times the text for scrolling must be blitted to create the continuation effect
            if self.layout_name in [
                "SCROLL_LEFT_CONTINUOUS",
                "SCROLL_RIGHT_CONTINUOUS",
            ]:
                self.scroll_repeats = (
    ...
    def show(self, surf, pos=(0, 0)):
        ...
        self.txt_surf.fill(
            (0, 0, 0, 0)
        )  # Last 0 indicates alpha, i.e. full transparency

        if self.layout_name == "TEXT_RIGHT":
            if self.font_bck_color:
                self.txt_surf.blit(
                    self.fnt_bck_surf,
                    (int(self.txt_surf_dim.width - self.fnt_txt_surf_dim.width), 0),
                )
            self.txt_surf.blit(
                self.fnt_txt_surf,
                (int(self.txt_surf_dim.width - self.fnt_txt_surf_dim.width), 0),
        ...
        if self.layout_name == "TEXT_LEFT":
            if self.font_bck_color:
                self.txt_surf.blit(self.fnt_bck_surf, (0, 0))
        ...
        if self.layout_name == "TEXT_CENTRE":
            if self.font_bck_color:
                self.txt_surf.blit(
                    ...
                    (
                        int(
                            self.txt_surf_dim.width // 2
                            - self.fnt_text_surf_dim.width // 2
                        ),
                        0,
                    ),
                )
            self.txt_surf.blit(
                ...
                (
                    int(
                        self.txt_surf_dim.width // 2 - self.fnt_txt_surf_dim.width // 2
                    ),
                    0,
                ),
            )
        ...
        if self.layout_name == "SCROLL_LEFT":
            ...
            if self.font_bck_color:
                self.txt_surf.blit(self.fnt_bck_surf, (int(self.scroll_offset), 0))
        ...
        if self.layout_name == "SCROLL_LEFT_CONTINUOUS":
            ...
            for i in range(self.scroll_repeats):
                if self.font_bck_color:
                    self.txt_surf.blit(
                        self.fnt_bck_surf,
                        (int(i * self.fnt_txt_surf_dim.width + self.scroll_offset), 0),
                    )
                self.txt_surf.blit(
                    self.fnt_txt_surf,
                    (int(i * self.fnt_txt_surf_dim.width + self.scroll_offset), 0),
            ...
            for i in range(self.scroll_repeats):
                ...
                if self.font_bck_color:
                    self.txt_surf.blit(
                        ...
                        (
                            int(
                                self.txt_surf_dim.width
                                - (i + 1) * self.fnt_txt_surf_dim.width
                                + self.scroll_offset
                            ),
                            0,
                        ),
        ...
        surf.blit(
            self.txt_surf,
            (int(pos[0] + self.padding.left), int(pos[1] + self.padding.up)),
        )
...
class TextOutput:
    ...
    def prepare_surface(self):
        ...
        for i in range(
            self.buffer_offset,
            min([len(self.buffer), self.buffer_offset + self.display_lines]),
        ):
            # Create font object with given text and given color
            # TODO - to check if the self.prompt must be on the line below???
            (surface_line_tmp, rect_tmp) = self.font_object.render(
                self.prompt + self.buffer[i][0], self.buffer[i][1], None
            )
        ...
        self.surf_dim = pygame.Rect(
            ...
            0,
            self.width,
            (self.line_spacing * len(self.surf_lines))
            + self.padding.up
            + self.padding.down,
        )
    ...
    def show(self, surf, pos=(0, 0)):
        ...
        for i in range(len(self.surf_lines)):
            ...
            if self.font_bck_color:
                fnt_bck_surf = pygame.Surface(
                    (fnt_txt_surf_dim.width, fnt_txt_surf_dim.height)
                )
                ...
                self.txt_surf.blit(
                    ...
                    (
                        ...
                        int(
                            height
                            + self.line_spacing
                            - ((self.line_spacing - fnt_txt_surf_dim.height) // 2)
                            - fnt_txt_surf_dim.height
                        ),
                    ),
                )
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pl.K_PAGEUP:
                    self.buffer_offset = max(
                        [0, self.buffer_offset - self.display_lines]
                    )
                    self.prepare_surface()

                elif event.key == pl.K_PAGEDOWN:
                    self.buffer_offset = min(
                        [
                            max([0, len(self.buffer) - self.display_lines]),
                            self.buffer_offset + self.display_lines,
                        ]
                    )
                    self.prepare_surface()
                ...
                # On mouse roll button DOWN - one row down
                    ...
                    self.buffer_offset = min(
                        [max([0, len(self.buffer) - 1]), self.buffer_offset + 1]
                    )
                    self.prepare_surface()
    ...
    def write(self, text, color=None):
        ...
        for text_line in text.split("\n"):
            ...
            if text_line:
                ...
                text_line_parts = [
                    text_line[i : i + self.display_columns]
                    for i in range(0, len(text_line), self.display_columns)
                ]
    ...
    def get_max_height(self):
        ...
        """
        return (
            (self.line_spacing * self.display_lines)
            + self.padding.up
            + self.padding.down
        )
...
class TextInput:
    ...
    def __init__(self, console, width, config={}):
        ...
        #####
        self.surf_dim = pygame.Rect(
            0, 0, self.width, self.line_spacing + self.padding.up + self.padding.down
        )
        self.surf = pygame.Surface((self.surf_dim.width, self.surf_dim.height))
        ...
        #####
        (self.fnt_txt_surf, self.fnt_txt_surf_dim) = self.font_object.render(
            self.prompt + self.text, self.font_color, None
        )
        ...
        if self.font_bck_color:
            self.fnt_bck_surf_dim = self.fnt_txt_surf_dim
            self.fnt_bck_surf = pygame.Surface(
                (self.fnt_bck_surf_dim.width, self.line_spacing)
            )
            self.fnt_bck_surf.fill(self.font_bck_color)
        ...
        self.cursor_surf_dim = pygame.Rect(
            0, 0, int(self.font_size / 2 + 1), self.line_spacing
        )

        self.cursor_surf = pygame.Surface(
            (self.cursor_surf_dim.width, self.cursor_surf_dim.height)
        )
        ...
        self.cursor_blit_position = self.font_object.get_rect(
            self.prompt + self.text[: self.cursor_position]
        ).width
        ...
        self.fnt_txt_scroll_offset = min(
            0,
            int(
                self.txt_surf_dim.width
                - self.fnt_txt_surf_dim.width
                - self.cursor_surf_dim.width
            ),
        )
        ...
        for key in self.keyrepeat_counters:
            ...
            if self.keyrepeat_counters[key][0] >= self.repeat_keys_initial_ms:
                self.keyrepeat_counters[key][0] = (
                    self.repeat_keys_initial_ms - self.repeat_keys_interval_ms
                )
    ...
    def show(self, surf, pos=(0, 0)):
        ...
        if self.font_bck_color:
            self.txt_surf.blit(
                ...
                (
                    ...
                    int(
                        self.line_spacing
                        - ((self.line_spacing - self.fnt_bck_surf_dim.height) // 2)
                        - self.fnt_bck_surf_dim.height
                    ),
                ),
            )
        ...
        self.txt_surf.blit(
            ...
            (
                ...
                int(
                    self.line_spacing
                    - ((self.line_spacing - self.fnt_txt_surf_dim.height) // 2)
                    - self.fnt_txt_surf_dim.height
                ),
            ),
        )
        ...
        if self.cursor_visible:
            self.txt_surf.blit(
                ...
                (
                    ...
                    int(
                        self.line_spacing
                        - ((self.line_spacing - self.cursor_surf_dim.height) // 2)
                        - self.cursor_surf_dim.height
                    ),
                ),
            )
...
class Console(pygame.Surface):
    ...
    def __init__(self, app, width, config={}):
        ...
        for key in global_config:
            setattr(self, key, global_config.get(key))
        ...
        self.cli = (
            CommandLineProcessor(self.app, output=self.console_output)
            if self.console_output
            else CommandLineProcessor(self.app)
        )
        ...
        self.layout = (
            "INPUT_BOTTOM" if self.layout not in Console.LAYOUTS else self.layout
        )
        ...
        if self.animation:
            ...
            self.anim_layout = (
                self.animation[0]
                if len(self.animation) > 0 and self.animation[0] in Console.ANIMATIONS
                else "TOP"
            )
            ...
            if self.console_input and self.console_input.update(events):
                ...
                if self.console_output:
                    self.console_output.write(
                        self.console_input.get_text(), self.console_input.font_color
                    )
    ...
    def show(self, surf, pos=None):
        ...
        # expressed by delta parameters anim_dx and anim_dy.
            ...
            if (self.enabled and self.anim_perc < 100) or (
                not self.enabled and self.anim_perc > 0
            ):
                self.anim_perc = (
                    self.anim_perc
                    + (1 if self.enabled else -1)
                    * (current_time - self.anim_last_time)
                    * self.anim_velocity
                )
        ...
        if self.anim_perc > 0:
            ...
            if self.layout == "INPUT_BOTTOM":
                ...
                self.text_output_position = (
                    self.padding.left,
                    self.header_position[1]
                    + (self.console_header.get_height() if self.console_header else 0),
                )
                self.text_input_position = (
                    self.padding.left,
                    self.text_output_position[1]
                    + (self.console_output.get_height() if self.console_output else 0),
                )
...
def get_console_config(sample=None):
    ...
    if sample == 1:
        return {
            "global": {
                ...
                "bck_image": "assets/Textures/custom/blurred.jpg",
                "bck_image_resize": True,
                "bck_alpha": 150,
